...
{
    if this.hasSort do this._style |= LBS_SORT
    if this.multiSelection do this._style |= LBS_EXTENDEDSEL | LBS_MULTIPLESEL
    if this.multiColumn do this._style |= LBS_MULTICOLUMN
    if this.noSelection do this._style |= LBS_NOSEL
    if this.keyPreview do this._style |= LBS_WANTKEYBOARDINPUT
